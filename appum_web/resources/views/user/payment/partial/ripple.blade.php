    ...
    <div class="modal-body">
        <div class="row no-margin" id="card-payment">
            <div class="form-group col-md-12 col-sm-12">
                <label>Ripple Price ( 1 XRP = {{$ripple_response->last}} USD ) </label>
                <?php $total_amount = number_format((Session::get('amount')/$ripple_response->last),6); ?>
                <input name="amount" id="amount" value="{{$total_amount}}" type="text"  readonly class="form-control" >
            </div>
            <div class="form-group col-md-12 col-sm-12">
                <label>@lang('user.card.enter_your_transaction_id')</label>
                <input name="payment_id" id="transaction_id" onChange="checkpayment();" type="text" class="form-control" placeholder="@lang('user.card.enter_your_transaction_id')" />
